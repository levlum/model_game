            ...
            if (Physics.Raycast(ray, out hit, 50f, ~ignoreMe))
            {
                ...
                else if (hit.collider.name == "Floor")
                {
                    ...
                    if (Input.GetButton("Fire1"))
                    {
                        ...
                        {
                            push_object.position = new Vector3(hit.point.x, Mathf.Max(-1f, push_object.position.y),
                                hit.point.z);
                            push_object.GetComponent<Rigidbody>().isKinematic = false;

                        }

                        push_object.GetComponent<Rigidbody>()
                            .AddForce(Vector3.up * 100f * Time.deltaTime, ForceMode.Impulse);
                    }
                }
            }
        }
